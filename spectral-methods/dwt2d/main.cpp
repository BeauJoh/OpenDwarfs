...
void rgbToComponents(cl_mem d_r, cl_mem d_g, cl_mem d_b, unsigned char * h_src, int width, int height)
{
    ...
    size_t globalWorkSize[1] = {static_cast<size_t>(alignedSize)};
...
void launchFDWT53Kernel (int WIN_SX, int WIN_SY, cl_mem in, cl_mem out, int sx, int sy)
{
    ...
    // prepare grid size
	size_t globalWorkSize[2] = { static_cast<size_t>(gx*WIN_SX), static_cast<size_t>(gy*1)};
    size_t localWorkSize[2]  = { static_cast<size_t>(WIN_SX), 1};
